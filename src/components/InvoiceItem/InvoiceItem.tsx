...
export const InvoiceItem = ({ invoice, setModalEdit, setModalDelete, setActiveTooltip, activeTooltip }: IProps) => {
  ...
  return (
    <div data-testid="invoice-item" className={`${styles['content']} ${styles['animate-right']}`}>
          ...
          <div className={styles['content-item-icon']}>
            {item.description && (
              <>
                ...
                <img style={{ cursor: 'pointer' }} width={12} height={12} data-testid={`icon-tooltip-${index + 1}`} onClick={() => handleClickInfo(index)} src={Info} alt="Icon info" />
              </>
            )}
            <img style={{ cursor: 'pointer' }} width={12} height={12} data-testid={`icon-edit-${index + 1}`} onClick={() => setModalEdit(item)} src={Edit} alt="Icon edit" />
            <img style={{ cursor: 'pointer' }} width={18} height={18} data-testid={`icon-delete-${index + 1}`} onClick={() => setModalDelete(item)} src={Delete} alt="Icon delete" />
          </div>
